...
public class PriorityConsumerBug2 {
	public static void main(String[] args) {
		PriorityConsumerBug2 main = new PriorityConsumerBug2();
		main.run();
	}
	...
	private void runExample(Connection connection) throws Exception {
		Holder holder = createNewSessionConsumer(connection, 3);
		connection.start();
		...
		if (holder.connectionStopped) {
			System.out.println("Msg received " + holder.msgCount);
			createNewSessionConsumer2(connection);
		}
	}
	...
	private void createNewSessionConsumer2(Connection connection) throws Exception {
		Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
		Queue exampleQueue = session.createQueue("ExampleQueue");
		MessageConsumer consumer = session.createConsumer(exampleQueue, "color = 'RED'");

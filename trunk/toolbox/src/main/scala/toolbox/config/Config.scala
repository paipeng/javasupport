...
class Config(env: String) {
  val $ = PropertiesFile("conf/"+env+"/config.properties")
  val fooDao: FooDao = new FooDaoImpl($("maxBar").toInt)
}
...
class FooDaoImpl(val maxBar: Int) extends FooDao{
